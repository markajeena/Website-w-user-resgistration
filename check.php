        ...
        <body>
           ...
           $in2 = $_GET["in2"];
            ...
            <?php $sql = "SELECT h1.username user1, h1.hobby, h2.username user2, h2.hobby FROM hobby AS h1, hobby AS h2 WHERE h1.hobby=h2.hobby AND h1.username <> h2.username";
                   $query = mysqli_query($db,$sql); ?>
                   <?php foreach($query as $q){ ?>
                      <div><a><?php echo $q['user1']," - ", $q['user2'], " = ".$q['hobby']; ?></a></div>
                   <?php } ?>
                

            <p>Display all the users who never posted a blog:</p><br>
            <?php $sql = "SELECT username FROM user WHERE username NOT IN (SELECT DISTINCT username FROM blog)";
                   $query = mysqli_query($db,$sql); ?>
                   <?php foreach($query as $q){ ?>
                      <div><a><?php echo $q['username']; ?></a></div>
                   <?php } ?>
            ...
            <?php $sql = "SELECT username FROM user WHERE username NOT IN (SELECT DISTINCT username FROM comment)";
                   $query = mysqli_query($db,$sql); ?>
                   <?php foreach($query as $q){ ?>
                      <div><a><?php echo $q['username']; ?></a></div>
                   <?php } ?>
